---
title: "Nature Communications | 表型组-环境组协同解锁组学驱动的作物智能育种"
tldr: "Nature Communications | 表型组-环境组协同解锁组学驱动的作物智能育种 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzI0Mjg5ODI1Ng==&mid=2247546257&idx=1&sn=a4ae7f64006b408803172ea6c4cc012d&chksm=e81ef8fbd26bb4809bab14ab8aa531fde02e582b2c5"
date_created: 2026-07-28
date_modified: 2026-09-17
type: source
tags: [vault-import, research_tools_and_methods, article]
source_type: article
source_file: "[[ Nature Communications - 表型组-环境组协同解锁组学驱动的作物智能育种]]"
explored: false
confidence: medium
---

# Nature Communications | 表型组-环境组协同解锁组学驱动的作物智能育种

## Summary
This source card was generated from an existing Obsidian note. The original note remains unchanged at [[ Nature Communications - 表型组-环境组协同解锁组学驱动的作物智能育种]].

## Key Takeaways
- Original note: [[ Nature Communications - 表型组-环境组协同解锁组学驱动的作物智能育种]]
- Imported category: [[Research Tools and Methods]]
- Local path: `llm-wikid\raw\articles\科研\科研工具\高温表型工具\ Nature Communications - 表型组-环境组协同解锁组学驱动的作物智能育种.md`
- Preview: Nature Communications | 表型组-环境组协同解锁组学驱动的作物智能育种 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzI0Mjg5ODI1Ng==&mid=2247546257&idx=1&sn=a4ae7f64006b408803172ea6c4cc012d&chksm=e81ef8fbd26bb4809bab14ab8aa531fde02e582b2c5

## Concepts & Entities Mentioned
- [[Research Tools and Methods]]

## Counter-arguments
- This card is an index-level extraction, not a full critical reading of the source.
- The category assignment is based on deterministic keyword matching and should be reviewed by the user.

## Data gaps
- Full claim-level citation and biological interpretation have not yet been manually curated.
- Duplicate or near-duplicate source notes should be merged at the input-note level before deeper synthesis.
