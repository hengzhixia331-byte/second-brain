---
title: "都Agent时代了，我还是想分享给你这12个我最常用的Prompt。"
tldr: "都Agent时代了，我还是想分享给你这12个我最常用的Prompt。 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzIyMzA5NjEyMA==&mid=2647685346&idx=1&sn=0fc8dc62bcf67a466552d1204205ef31&chksm=f1c4daa88086d2c7edd7fc6cbfcb350c6bac2b3131f372da82dba7e"
date_created: 2026-08-23
date_modified: 2026-08-23
type: source
tags: [vault-import, ai_for_research, notes]
source_type: notes
source_file: "[[提问永不过时 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。]]"
explored: false
confidence: medium
---

# 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。

## Summary
This source card was generated from an existing Obsidian note. The original note remains unchanged at [[提问永不过时 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。]].

## Key Takeaways
- Original note: [[提问永不过时 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。]]
- Imported category: [[AI for Research]]
- Local path: `llm-wikid\raw\articles\学习AI\提示词与生图\提问永不过时 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。.md`
- Preview: 都Agent时代了，我还是想分享给你这12个我最常用的Prompt。 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzIyMzA5NjEyMA==&mid=2647685346&idx=1&sn=0fc8dc62bcf67a466552d1204205ef31&chksm=f1c4daa88086d2c7edd7fc6cbfcb350c6bac2b3131f372da82dba7e

## Concepts & Entities Mentioned
- [[AI for Research]]

## Counter-arguments
- This card is an index-level extraction, not a full critical reading of the source.
- The category assignment is based on deterministic keyword matching and should be reviewed by the user.

## Data gaps
- Full claim-level citation and biological interpretation have not yet been manually curated.
- Duplicate or near-duplicate source notes should be merged at the input-note level before deeper synthesis.
