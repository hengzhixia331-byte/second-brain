---
title: "玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→"
tldr: "玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→ 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzIzNDUyODUxMw==&mid=2247506709&idx=1&sn=68121dec574be2ea8ab32624e25f190b&chksm=e9b5469b3b30c87aea61d6eed22cda94a29fc756c56e590849a6b139808"
date_created: 2026-07-03
date_modified: 2026-09-17
type: source
tags: [vault-import, maize_yield_and_breeding, notes]
source_type: notes
source_file: "[[玉米育种知识 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→]]"
explored: false
confidence: medium
---

# 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→

## Summary
This source card was generated from an existing Obsidian note. The original note remains unchanged at [[玉米育种知识 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→]].

## Key Takeaways
- Original note: [[玉米育种知识 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→]]
- Imported category: [[Maize Yield and Breeding]]
- Local path: `llm-wikid\raw\articles\科研\科研工具\了解-可能用到\玉米育种知识 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→.md`
- Preview: 玉米 种植密度（亩穗数）、穗子大小（穗粒数）、千粒重与产量→ 笔记同步助手 来源 [原文链接](https://mp.weixin.qq.com/s?__biz=MzIzNDUyODUxMw==&mid=2247506709&idx=1&sn=68121dec574be2ea8ab32624e25f190b&chksm=e9b5469b3b30c87aea61d6eed22cda94a29fc756c56e590849a6b139808

## Concepts & Entities Mentioned
- [[Maize Yield and Breeding]]

## Counter-arguments
- This card is an index-level extraction, not a full critical reading of the source.
- The category assignment is based on deterministic keyword matching and should be reviewed by the user.

## Data gaps
- Full claim-level citation and biological interpretation have not yet been manually curated.
- Duplicate or near-duplicate source notes should be merged at the input-note level before deeper synthesis.
